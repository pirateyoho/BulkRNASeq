...
Option 2: Import a normalized count matrix in CSV format.
```{r dataImportNormCts}
...
gathered_countsSubset1 <- countsSubset1 %>%
  gather(colnames(countsSubset1)[2:104], key = "sample_name", value="normalized_counts")
...
NOTE: This code for variance stabilized transformation depends on an imported dds object. If you only imported a CSV of normalized counts, skip these code chunks with EVAL=FALSE.
...
ggplot(gathered_countsSubset, aes(x=gene_name, y=normalized_counts, color=phenotype, shape=phenotype)) +
  ...
  # display stats; replace wilcox_test with t_test based on results of normalization test
  geom_pwc(aes(group=breed), method = "wilcox_test", dodge=0.6, tip.length=0, hide.ns = TRUE, label = "p.signif") +
