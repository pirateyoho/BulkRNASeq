...
```{r data-export1}
# LMO4 only vs GATD3A, significant results only
gse1_msigdb_df <- as.data.frame(gse1_msigdb_all)
write.csv(gse1_msigdb_df, file="../03_output/240425_gseaAllMSigDBGeneSets_LMO4OnlyvsGATD3AOnly.csv") # Replace with desired output path and filename

# GATD3A only vs LMO4, significant results only
gse2_msigdb_df <- as.data.frame(gse2_msigdb_all)
write.csv(gse2_msigdb_df, file="../03_output/YYMMDD_gsea_allMSigDBGeneSets.csv") # Replace with desired output path and filename
```

## Using specific gene sets of interest from MSigDB

### Upload gmt files
Input: gene sets in .gmt format, downloaded from MSigDB.
```{r mutliple-gmt-import, warning=FALSE}
## Input GMT files for specific gene set(s) of interest.
# For importing and combining multiple GMT files in the same directory

setwd("../01_input/GeneSets") # set working directory to where cell of origin .gmt files are located
...
# Read in and print a list of gene sets included in this analysis and their description
gmtInfo <- read.table("GeneSetDescriptions.csv", sep=',', header=TRUE)
kable(gmtInfo)
...
### Enrichment analysis
```{r gsea_cellcycle, warning=FALSE}
...
### Dotplots
```{r dotplot-cellcycle, warning=FALSE}
gse1 %>%
...
### Ridgeline plots
```{r ridgeplot-cellcycle, message=FALSE, warning=FALSE}
...
sig_up_genes_LMO4 <- names(sig_genes_LMO4)[sig_genes_LMO4 > 1]
sig_up_genes_GATD3A <- names(sig_genes_GATD3A)[sig_genes_GATD3A > 1]
```
